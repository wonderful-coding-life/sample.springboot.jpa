...
public class CustomerControllerThruServerTests {
	...
	public void postCustomer() throws Exception {
		Customer customer = restTemplate.postForObject("http://localhost:" + port + "/api/customer?name=홍길동&address=서울시 강남구 대치1동&contact=010-1111-1111", null, Customer.class);
	...
	public void putCustomer() throws Exception {
		Customer customer = restTemplate.postForObject("http://localhost:" + port + "/api/customer?name=홍길동&address=서울시 강남구 대치1동&contact=010-1111-1111", null, Customer.class);
		restTemplate.put("http://localhost:" + port + "/api/customer/" + customer.getId() + "?name=홍길철&address=서울시 강남구 대치2동&contact=010-1111-1112", null);
		Customer actual = repository.findById(customer.getId()).orElseThrow(() -> new IllegalStateException());
